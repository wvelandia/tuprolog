...
public class JavaLibrary extends Library {
	...
	public boolean java_call_3(Term objId, Term method_name, Term idResult) throws JavaException{
		...
		Signature s = parseArg(method);
		Object[] args = s.values;
		Object res = null;
...
/*		for (int i = 0; i< method.getArity(); i++){
			if (method.getArg(i) instanceof Var){
				getEngine().warn("Variable on arg: " +i );
				throw new JavaException(new IllegalArgumentException(method.getArg(i)
						.toString()));
			}
			Struct arg = (Struct) method.getArg(i);
			if (arg.getArity() == 1 && arg.getName().equals("class")) {
				try {
					Class<?> cl = Class.forName(alice.util.Tools
							.removeApices(arg.getArg(0).toString()));
					args.add(cl.newInstance());
				}catch (ClassNotFoundException e){
					getEngine().warn("Class not found :"+arg.getArg(0).toString());
					throw new JavaException(e);
				} catch (InstantiationException e) {
					// TODO Auto-generated catch block
					getEngine().warn("Class Instantiation Error" + arg.getArg(0).toString());
					throw new JavaException(e);
				} catch (IllegalAccessException e) {
					getEngine().warn("Class Illegal Access Error" + arg.getArg(0).toString());
					// TODO Auto-generated catch block
					throw new JavaException(e);
				}
			}else{
				String argName = alice.util.Tools.removeApices(method.getArg(i).toString());
				if (currentObjects.containsKey(argName)){
					args.add(currentObjects.get(argName));
				}else{
					args.add(argName);
				}
			}
...
*/
		...
		try {
			if (obj != null){
				res = callMethod(obj, methodName, args);
...
//							System.out.println("call method : "+ cl.getCanonicalName()+" name : "+methodName +" args"+args.toString());
//							callMethod(cl.newInstance(), methodName, args);
//						} catch (ClassNotFoundException ex) {
...
//						//                    System.out.println("methodName: "+methodName +" Class:"+id.getName()+" args : "+args.toString());
//						callMethod(objId.toString(), methodName, args);
//					}
					if (objId.isCompound()) {
	                    Struct id = (Struct) objId;
	                    if (id.getArity() == 1 && id.getName().equals("class")) {
	                        try {
	                            Class<?> cl = Class.forName(alice.util.Tools
	                                    .removeApices(id.getArg(0).toString()));
	                            Method m = cl
	                                    .getMethod(methodName, s.getTypes());
	                            m.setAccessible(true);
	                            res = m.invoke(null, s.getValues());
	                        } catch (ClassNotFoundException ex) {
	                            // if not found even as a class id -> consider as a
	                            // String object value
	                            getEngine().warn("Unknown class.");
	                        ...
	                        Method m = java.lang.String.class.getMethod(methodName,
	                        		s.getTypes());
	                        m.setAccessible(true);
	                        res = m.invoke(objName, s.getValues());
	                    }
				} else {
					// the object is the string itself
					callMethod(objId.toString(), methodName, args);
			...
			getEngine().warn("Invocation Error");
			throw new JavaException(e);
		} 
	 ...
	 * attempts to call a method of class srcClass.
	 * @param srcClass
	 * @param method
	 * @param args
	 * @return
	...
	private Object callMethod(Object srcClass,String method,Object[] args) throws NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException{
		Class<?> cl = srcClass.getClass();
		Method[] methods = cl.getMethods();
		//System.out.println("callMethod : ")
		Method choice = null;
		for (Method m : methods){
			Class<?>[] par = m.getParameterTypes();
			if (par.length == args.length && m.getName().equals(method)){
				boolean match = true;
				for(int i = 0; match && i < par.length; i++){
					if(args[i] == null){
						match = false;
						break;
					}
					System.out.println("Class arg : "+par[i].getCanonicalName()+" cast to : "+args[i].getClass().getCanonicalName());
					System.out.println("check : "+par[i].isAssignableFrom(args[i].getClass()));
					if (!par[i].isAssignableFrom(args[i].getClass())){
						match = false;
						break;
					}
				}
				if (match){
					choice = m;
					System.out.println("invoco "+ choice.toString());
					break;
				}
			}
		}
		if (choice != null){
			...
			return choice.invoke(srcClass,args);		
		}else{
