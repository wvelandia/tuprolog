...
public class SocketLib extends Library implements ISocketLib {
	...
	private LinkedList<Reader> readers;					// Active readers
	...
	public boolean aread_from_socket_2(Term Socket, Struct Options) throws PrologError {
		...
		if (!((AbstractSocket) Socket.getTerm()).isClientSocket()) { // Only Client_Sockets can receive data
			...
			if (r == null) {
				synchronized (this) {
					readers.add(new Reader(sock, this.getEngine()));
					r = readers.getLast();
				}
			}
	 ...
	 */
	private Reader readerExist(Socket s) {
	...
	public void onSolveEnd(){
		...
		clientSockets = new LinkedList<Socket>();
		for(Reader r:readers)r.stopRead();
	...
	private class Reader extends Thread {
		...
		protected Reader(Socket socket, Prolog mainEngine) {
			this.socket = socket;
