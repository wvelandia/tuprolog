...
public class ConsoleDialog
	extends JPanel
	implements OutputListener, InformationToDisplayListener, PropertyChangeListener, MouseListener, ChangeListener
{
	...
	private void showSolution(SolveInfo info)
	{
		...
		try
		{
			if (info.isSuccess())
			{
				...
				for (Iterator i = info.getBindingVars().iterator(); i.hasNext();) {
					Var v = (Var) i.next();
					if (!v.isAnonymous())
						bindings.add(v);
				}
