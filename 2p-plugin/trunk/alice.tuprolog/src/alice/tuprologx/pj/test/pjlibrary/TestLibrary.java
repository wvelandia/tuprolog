...
public abstract class TestLibrary {
    ...
    @TRACE
    @PrologMethod (
        clauses={"new_list(I):-new_object('java.util.ArrayList', [], L), L <- add(one), L <- add(two), L <- add(three), L <- iterator returns I."})
	public abstract <$L extends JavaObject<Term<?>>> $L new_list();

    @TRACE
    @PrologMethod (
        clauses={"prova(Iterable, Element) :-Iterable <- hasNext, prova2(Iterable, Element).",
                 "prova2(Iterable, Element) :- Iterable <- next returns Element.",
                 "prova2(Iterable, Element) :- Iterable <- hasNext, prova2(Iterable, Element). ",
"test(E, I) :- prova(I, E)."})
