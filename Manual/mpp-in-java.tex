...
\begin{description}
  \item [Object creation]
        ...
        \color{red}
        To make such a binding permanent, that is, to ``keep alive'' the binding between a Java object and a Prolog term beyond the current query, so as to exploit it in another, subsequent demonstration, the \texttt{register} predicate is provided;
        \normalcolor
...
%-----------------------------------------------------------------------
\color{red}
\subsubsection{Registering object bindings (to be implemented)}
...
\normalcolor
Binding registration can be performed also on the Java side, as detailed in Section \ref{sssec:register(Java)}.
...
%-----------------------------------------------------------------------
\section{Augmenting Prolog from Java:\\developing new libraries}
...
\color{red}
The two approaches, however, are not completely identical, at least if \texttt{2p.jar} is used to start the \tuprolog{} GUI (or the console-based CUI) from which the loading is performed.
In fact, library loading from Prolog suffers from some limitations due to the Java reflection mechanism used internally by \tuprolog{} to locate the library,\footnote{%
  The culprit is the \texttt{Class.forName} method.
} which does not search outside the JAR. So, only libraries packed into the same \tuprolog{} JAR can be found and loaded from Prolog via the \texttt{load\_library/2} predicate.
To avoid the problem, the JAR must be unpacked: then, any library can be found and loaded regularly from Prolog, too.
