...
While their reading is recommended to everyone, the reader wishing only to exploit \tuprolog{}.NET in its current version can safely bypass them and jump directly to Section \ref{sec:dotnet-basics}.
...
\begin{itemize}
  ...
  \item \textit{enumerations} are mapped onto classes extending \texttt{cli.System.Enum}, with static fields with integer values for each possible value of the .NET enumerative type (\color{red} in the future, Java \texttt{enum}s will be used instead\normalcolor);;
  ...
  \item \textit{params} is mapped onto an array of \texttt{Object}s (\color{red} in the future, generics will be used instead of raw Objects\normalcolor);
