...
Analogously to the Java case (see Section \ref{ssec:java-exceptions-in-tuprolog}), .NET exceptions possibly raised during the execution of methods on .NET objects accessed from the Prolog world need to be captured and handled, too, yet without any mixing with the Prolog semantics.

\color{red}
Accordingly, such exceptions are handled in \tuprolog{}.NET via two further, \textit{ad hoc} predicates, \texttt{oo\_throw/1} and \texttt{oo\_catch/3}, which constitute the .NET counterpart of JavaLibrary's \texttt{java\_throw/1} and \texttt{java\_catch/3}.
\normalcolor
%
