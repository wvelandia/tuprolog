...
\end{itemize}
%
Each thread is committed to solve a specific goal, and is identified by a unique identifier. From the model viewpoint, \tuprolog{} distinguishes the \textit{execution environment} and the \textit{execution context}: the execution environment includes the information to be shared among multiple threads (like predicates, facts, primitives, flags, operators, etc.) while the execution context embeds the private information generated by each thread at run time, like its choicepoints, unifications, etc.

%The execution context is represented as an \texttt{EngineManager} instance, that encapsulates the \tuprolog{} finite-state engine; the execution environment, instead, is mapped onto the other traditional \tuprolog{} managers (\texttt{TheoryManager}, \texttt{LibraryManager}, \texttt{OperatorManager}, \texttt{FlagManager}, \texttt{PrimitiveManager}). The new \texttt{ThreadManager} takes care of creating a new thread, with its execution context, for each new query: so, the Thread manager holds the list of all its threads, that are \texttt{EngineRunner} instances.

In this model, multiple threads cooperate -- at subsequent times -- to find the solutions of a query: more precisely, while each thread computes a single solution and then terminates, its execution context is not discarded, so as to have it available if an alternative solution is requested. In this case, a new thread is generated and associates to the existing execution context, to find the next solution.
So, the unique thread ID seen by the user actually refers to an execution context (hence a family of threads), rather than to a single thread, since different physical threads re-use that ID at different times.
...
\item \bti{thread\_create/2}\\
    \noindent\bt{thread\_create(+Query, -ThreadID)} is true if a new thread can be created to solve the given \bt{Query}; the unique ID of the newly-thread thread is unified with \bt{ThreadID}.
...
\item \bti{thread\_execute/2}\\
    \noindent\bt{thread\_execute(+Query, +ThreadID)} is the backtrackable version of \bt{thread\_create/2}. It must be followed by some read operation to retrieve the subsequent solutions computed by the thread.
    ...
    \template{thread\_execute(+term, +number)}
...
\item \bti{thread\_send\_msg/2}\\
    \noindent\bt{thread\_send\_msg(+Msg, +QueueOrThreadID)} inserts a message in the queue identified by \texttt{QueueOrThreadID}. As a side effect, any thread suspended on that queue is awakened.
...
\item \bti{thread\_remove\_msg/2}, \mbox{~~~}\bti{thread\_peek\_msg/2}\\
    \noindent\bt{thread\_remove\_msg(?Msg, +QueueOrThreadID)} and\\
     \bt{thread\_peek\_msg(?Msg, +QueueOrThreadID)} look into the queue identified by \texttt{QueueOrThreadID}, for a message unifying with \bt{Msg}. If none exists, both primitives fail. Upon success, \bt{thread\_remove\_msg} removes the message from the queue, while \bt{thread\_peek\_msg} leaves it there.
...
\item \bti{msg\_queue\_size/2}\\
    \noindent\bt{msg\_queue\_size(?Size, +QueueOrThreadID)} unifies the number of elements in the queue \texttt{QueueOrThreadID} with \texttt{Size}.
...
start(X):- msg_queue_create('MyQueue'), 
           thread_create(child(X),ID),
           thread_sleep(500), 
           thread_send_msg( m('important message'), 'MyQueue'), 
           thread_join(ID, child(X)).
           
child(X):- thread_get_msg( m(X),'MyQueue').
\end{verbatim}
%
...
\begin{verbatim}
start(X):- thread_create(thread1(ID1),  ID1), 
           thread_sleep(200),
           thread_create(thread2(ID1),  ID2), 
           thread_create(thread3(ID1,X),ID3), 
           thread_read(ID3, _),
           write('Father: done'). 
           
thread1(ID):- tell('threadLog.txt'),
              write('ID1: waiting for message'), 
              thread_get_msg( m(X),ID), 
              write('ID1: message retrieved').

thread2(ID):- thread_send_msg( m('critical message'), ID), 
              write('ID2: message sent').		

thread3(ID,X):- thread_peek_msg( m(X), ID),
...
In this producer/consumer example, the consumer's goal is to retrieve only the \textit{last} solution of the query \texttt{has\_child(bob,X)}, while the producer computes all the solutions to this query; the father coordinates the two tasks (the execution flow diagram is reported in Figure \ref{fig:thread-exampleMutex} on page \pageref{fig:thread-exampleMutex}).
...
\begin{verbatim}
start :- thread_create( has_child(bob,X), ID1),
         mutex_lock('mutex'),
         thread_create( read_child(ID1,X), ID2), 
         loop(1,5,1, ID1), 
         mutex_unlock('mutex').
...
La chiamata alla funzione loop(from,to,inc,info) consiste in un ciclo iterativo in cui bisogna specificare l’indice da cui partire, il suo valore iniziale, l’incremento desiderato e i parametri che eventualmente devono essere trasferiti da una chiamata all’altra, come in questo caso l’identificatore del thread. Il ciclo ha lo scopo di chiamare delle thread\_next\_sol/1 ripetutamente fino a quando le soluzioni della query non sono esaurite. 
Il thread padre poi genera un secondo figlio con il ruolo di lettore dell’ultima soluzione computata dal thread ID1.

SENZA MUTEX, non sarebbe possibile prevedere quale soluzione sia prelevata tra alex, anna o maria, perché ciò dipenderebbe dalle velocità relative del processo lettore e del padre. Infatti, se il padre non aspetta che il figlio computi tutte le soluzioni, ma deposita semplicemente le richieste di ulteriori computazioni continuando poi la sua esecuzione, il figlio lettore viene generato durante la fase di computazione del produttore e di conseguenza preleva la prima soluzione disponibile. 
