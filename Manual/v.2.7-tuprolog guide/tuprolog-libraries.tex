...
\begin{itemize}
  ...
  \item predicates for the communication among threads;
  \item predicates for the mutual exclusion among threads;
  \item backtrackable predicates. 
\end{itemize}
%
Since the underlying architecture has been changed in-depth in some points to support multi-threading, 
ThreadLibrary is loaded by default in any \tuprolog{} engine.

%ED
\color{red}
aggiungere due parole sul modello di thread usato, e i loro stati
...
\item \bti{thread\_join/2}\\
    ...
    \exception{error(type\_error(ExpectedType, Culprit), type\_error(\\
    Goal, ArgNo, ExpectedType, Culprit))} if \texttt{ThreadID} is not an integer. \texttt{Goal} is the goal where the problem occurred, \texttt{ExpectedType} is the expected type (integer),  \texttt{Culprit} is the wrong term actually found.
