...
\item \bti{tcp\_socket\_server\_open/3}\\
    ...
    \textbf{\exception{error(instantiation\_error, instantiation\_error(\\
    Goal, ArgNo))} if \texttt{Category} is a variable. \texttt{Goal} is the goal where the problem occurred, \texttt{ArgNo} indicates the argument that caused the problem (obviously, \texttt{1}).}
...
\item \bti{udp\_socket\_open/2}\\
    \noindent\bt{udp\_socket\_open(+Address, -Socket)} is true iff \bt{Address} represents a valid Internet address in the \texttt{\textit{Address}:\textit{Port}} form, a server is waiting for incoming connection at that address, and \texttt{Socket} is unified with a newly-created socket.
...
\item \bti{udp\_socket\_close/1}\\
    \noindent\bt{udp\_socket\_close(+DatagramSocket)} is true iff \bt{DatagramSocket} represents a valid server socket; as a side effect, the socket is closed.
...
\item \bti{udp\_send/3}\\
    \noindent\bt{udp\_send(+Socket, +Msg, +AddressTo)} is true iff \bt{Socket} represents a valid datagram socket with an open associated output stream, \texttt{Msg} is a valid term representing the message to be sent, and \texttt{AddressTo} represents the destination address in the \texttt{\textit{Address}:\textit{Port}} form; as a side effect, the message is sent onto the stream.
...
\item \bti{udp\_receive/4}\\
    \noindent\bt{udp\_receive(+Socket, -Msg, +AddressTo, +Options)} is true iff \\
    \bt{Socket} represents a valid datagram socket with an open associated input stream, \texttt{AddressTo} represents the destination address in the \texttt{\textit{Address}:\textit{Port}} form, and \texttt{Options} is a possibly-empty option list; currently, the only available option is \texttt{max\_msg\_size(+\textit{Size})}, whose default value is 4096 bytes. Upon reception of a message, as a side effect, the \texttt{Msg} term is unified with the incoming message.
