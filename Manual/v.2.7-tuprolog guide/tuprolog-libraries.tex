...
So, the unique thread ID seen by the user actually refers to an execution context (that is, to a family of threads), rather than to a single thread, since different physical threads re-use that ID at different times.
%
More detailed information about the internal organisation and thread handling is reported in the box on page \pageref{box:thread-internals}.
...
However, the example shows how to create a pair of threads (\texttt{thread\_create} primitive), each identified by a unique identifier and devoted to solve a given goal, and how such identifier is reused to get the solution (\texttt{thread\_read} primitive).
...
noindent The following predicates are provided to create and delete threads:
