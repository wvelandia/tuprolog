...
As of \tuprolog{} 2.7.2, an alternative approach is provided by the \textit{Spy Frame} window, which can be opened clicking on the top-right, tree-like button (Figure \ref{fig:gui-spyframe}).
This window makes it possible to graphically reproduce the solving process of a query step-by-step, \textit{after the query has been solved}: so, while the above debug view operates in real time, this view operates off-line; still, it can be very effective to understand where and how something went wrong.
Each time the Next button is pressed, the simulation advances of \textit{N} steps, where \textit{N} is the number shown in the adjacent textfield (default, obviously, 1).
