...
Of course, the new library must be in the current class path for \tuprolog{} to find it; alternatively, the \textit{Browse..} button can be used to locate a library class anywhere in the file system.
%
Quite clearly, any further class needed by a library must be in the current classpath, too, or the library will not be added to the manager/loaded into the engine.
